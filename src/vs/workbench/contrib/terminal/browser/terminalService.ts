...
export class TerminalService implements ITerminalService {
	...
	private _shouldReviveProcesses(reason: ShutdownReason): boolean {
		...
		switch (this.configHelper.config.persistentSessionReviveProcess) {
			case 'onExit': {
				if (reason === ShutdownReason.CLOSE && this._nativeDelegate && this._nativeDelegate) {

				} else if (await this.nativeHostService.getWindowCount() > 1 || isMacintosh) {
				}
				console.log('onExit reason?', reason);
				// TODO: Check if it's the last window and linux/windows
